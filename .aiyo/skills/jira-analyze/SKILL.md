...
□ write_artifact 记录从confluence获取的页面ID和模块知识（confluence_notes.md）
□ write_artifact 记录日志发现（log_findings.md）
□ write_artifact 记录相关代码发现（code_findings.md）
□ write_artifact 记录分析过程（analysis_process.md）
□ 参考 related_cases 历史案例
...
4. 最终传给 exit_analyze 的必须是完整 conclusion 文本，不是 JSON 草稿
```
...
- `attachments`: 附件下载结果
- `logs_index`: 已识别为日志类的附件列表
- `existing_artifacts`: 之前写过的笔记；如果存在，先 `read_artifacts` 避免重复劳动
- `related_cases`: 历史相似案例，后续用于交叉验证

执行要求：
- 先快速判断 issue 对应的模块、现象、时间线、是否有日志附件
- 如果 `existing_artifacts` 不为空，先阅读已有笔记再继续
- 如果附件下载失败或没有日志，在 `analysis_process.md` 中明确记录信息缺口
...
优先分析 `logs_index` 中的日志文件。带着文档关键词去筛日志，不要盲读全部附件。
...
用 `write_artifact` 写 `log_findings.md`，至少记录：
- 关键错误行或关键日志片段
- 时间线或调用链
- 每条证据对应哪个模块知识点
- 哪些现象已确认，哪些只是待验证假设
...
用 `write_artifact` 写 `code_findings.md`，至少记录：
- 可疑文件路径 / 类 / 函数
- 关键代码分支或状态判断
- 日志与代码的对应关系
- 最可能的失效路径
...
在调用 `exit_analyze` 前，先整理一份完整的 `conclusion` 文本。建议至少包含：
- 问题概述
- 根因判断
- 关键证据
- 代码定位
...
注意：
- 传入的是完整结论文本，不是 `analysis_struct`
- 在调用前确认 `confluence_notes.md`、`log_findings.md`、`code_findings.md`、`analysis_process.md` 已写好
...
2. **4 个笔记要补齐**：`confluence_notes.md`、`log_findings.md`、`code_findings.md`、`analysis_process.md`。
3. **根因必须有证据链**：至少能串起 Jira 现象、日志片段、代码路径中的两个以上。
4. **历史案例只能辅助**：不能用历史结论替代本次 issue 的证据。
5. **exit_analyze 是唯一出口**：所有结果必须最终收敛为 `conclusion` 并通过它落盘。
